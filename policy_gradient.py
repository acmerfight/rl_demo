...
class PolicyGradientAgent:
    ...
    def update_policy(self) -> None:
        ...
        for t in range(len(self.states_history)):
            ...
            # 假设在状态"A"，有两个可用动作"right"和"down"：
            # "right"当前概率为0.6，我们选择了这个动作，得到回报G=10
            # "down"当前概率为0.4
            # 参数更新计算：
            # "right"（选中的动作）：
            # 梯度 = 1 - 0.6 = 0.4
            # 更新值 = 0.1(学习率) × 0.4(梯度) × 10(回报) = 0.4
            # 新参数 = 旧参数 + 0.4
            # "down"（未选中的动作）：
            # 梯度 = -0.4
            # 更新值 = 0.1 × (-0.4) × 10 = -0.4
            # 新参数 = 旧参数 - 0.4
            # 更新后，"right"动作的偏好值增加，"down"动作的偏好值减少，在下一次选择时，选择"right"的概率会更高。
...
def explain_policy_gradient_step(
    state, action, reward, action_probs, gradient, policy_update
):
    """解释策略梯度算法的更新步骤"""
    print("\n===== 策略梯度更新步骤说明 =====")
    ...
    for episode in range(episodes):
        ...
        while not done and steps < 20:  # 防止无限循环
            ...
            if verbose >= 2 and episode < 3:
                ...
                print(
                    f"状态: {state} -> 动作: {action} -> 新状态: {next_state}, 奖励: {reward}"
                )
                ...
                policy_update = (
                    agent.learning_rate * gradient * reward
                )  # 简化版，仅使用即时奖励
                explain_policy_gradient_step(
                    state, action, reward, action_probs, gradient, policy_update
                )
...
def test_agent(env, agent):
    ...
    while not done and steps < 10:
        ...
        # 显示当前状态
        print(
            f"状态: {state} -> 动作: {action} (概率: {action_probs[action]:.4f}) -> 新状态: {next_state}, 奖励: {reward}"
        )
        env.render()
        sleep(1)
...
def explain_policy_gradient():
    ...
    print("\n===== 策略梯度算法解释 =====")
    print(
        "策略梯度是一种直接学习策略函数的强化学习方法，不需要通过值函数间接得到策略。"
    )
    print("\n核心思想:")
...
if __name__ == "__main__":
    ...
    # 训练智能体
    train(env, agent, episodes=50000, verbose=2, visualization_speed=0.5)
